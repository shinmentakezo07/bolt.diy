...
export interface UseLocalModelHealthReturn {
  ...
  getHealthStatus: (
    provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA',
    baseUrl: string,
  ...
  stopMonitoring: (provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string) => void;
  performHealthCheck: (provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string) => Promise<void>;
  isHealthy: (provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string) => boolean;
...
export function useLocalModelHealth(options: UseLocalModelHealthOptions = {}): UseLocalModelHealthReturn {
  ...
  const getHealthStatus = useCallback((provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string) => {
    return localModelHealthMonitor.getHealthStatus(provider, baseUrl);
  }, []);

  // Start monitoring a provider
  const startMonitoring = useCallback(
    (provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string, interval?: number) => {
  ...
  const stopMonitoring = useCallback((provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string) => {
  ...
  const performHealthCheck = useCallback(
    async (provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string) => {
  ...
  const isHealthy = useCallback(
    (provider: 'Ollama' | 'LMStudio' | 'OpenAILike' | 'NVIDIA', baseUrl: string) => {
